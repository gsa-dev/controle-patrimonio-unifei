...
<html>
    ...
    <body>
        ...
        $nSerie = $_POST["nSerie"];
        $sql = "SELECT * FROM patrimonio WHERE numero_serie LIKE '%" . $nSerie . "%'";
        $result = $conn->query($sql);
        ...
        if (empty($dados)) {
            ...
            $_SESSION["id_patrimonio"] = $dados["id"];
            $w = $_SESSION["id_patrimonio"];

            $verifica_emprestimo = mysqli_query($conn, "SELECT * FROM emprestimo WHERE id_patrimonio = $w");
            while ($p = mysqli_fetch_array($verifica_emprestimo)) {
                if (isset($p["id_patrimonio"]) && $p["emprestado"] == 1) {

                    echo("<script type='text/javascript'> alert('Item ja emprestado!'); location.href='pagina_principal.php';</script>");
                }
            }
        }
        ...
        <nav class="w3-sidenav w3-collapse w3-white w3-animate-left w3-card-2" style="z-index:3;width:250px;">
            ...
            <a href="pagina_principal.php" class="w3-light-grey w3-medium">Emprestimo</a>
        ...
        <div class="w3-main" style="margin-left:250px;">
            ...
            <div class="w3-container w3-padding-32" style="padding-left:32px">
                ...
                <h2 style="text-align:center">Emprestimo</h2>
                <br><br>

                <div action="form.asp" class="w3-card-4">
                    <form class="w3-container" action="cadastra_emprestimo.php" method="post">
                        <p>
                            <label class="w3-label w3-validate" style="color:black">Item</label></p>
                        <input class="w3-input w3-border" type="text" style="width:90%" name="item" value="<?php echo $dados["descricao_fabricante_modelo"]; ?>"disabled>
                        <p>
                            <label class="w3-label w3-validate" style="color:black">Numero de Serie</label></p>
                        <input class="w3-input w3-border" type="text" style="width:90%" name="nSerie" value="<?php echo $dados["numero_serie"]; ?>" disabled>
                        <p>
                            <label class="w3-label w3-validate" style="color:black">Data</label></p>
                        <input class="w3-input w3-border"  type="date" style="width:50%" name="data" value="<?php echo $data; ?>" disabled>
                        <p>
                            <label class="w3-label w3-validate" style="color:black">Nome</label></p>
                        <input class="w3-input w3-border" type="text" style="width:90%" name="nome" required>
                        <p>
                            <label class="w3-label w3-validate" style="color:black">Condicoes</label></p>
                        <input class="w3-input w3-border" type="text" style="width:90%; height:150px" name= "condicoes" required>
                        <p>
                            <button class="w3-btn w3-section w3-ripple button button1">Salvar</button></p>
                    </form>
                </div>
